...
$('body').on('click', 'a[data-youtube]', function(e) {
    ...
    $('body').append('<div role="dialog" class="overlay"><button id="overlay-close" class="btn-round-light icon-x" data-targetId="'+videoId+'">Close</button><div class="video-wrapper"><div class="video-embed"><iframe id="vid-'+videoId+'" width="560" height="315" type="text/html" src="https://www.youtube.com/embed/'+videoId+'?rel=0&amp;autoplay=1" frameborder="0" allowfullscreen></iframe></div></div></div>').addClass('overlay-open');
...
// handler for overlay close button
$('body').on('click keydown', '#overlay-close, .overlay', function(event) {
    // id of view video button clicked
    var returnFocusLink = '[data-youtube='+$(this).attr('data-targetId');
    if (event.keyCode === undefined || event.keyCode == 13 || event.keyCode == 27) {
        // undefined for click, 13 for enter key, 27 for esc key
        event.preventDefault();
